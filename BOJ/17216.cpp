...
// 어떤 수열의 감소하는 부분 수열 중 합이 가장 큰 결과는?

// 가장 긴 감소하는 부분 수열과 같은 원리 적용 - n^2를 돌면서 각 자리에 대한 최적의 dp 저장
// 그런데 여기는 저장되는 dp 값이 달라짐 - 해당 위치까지의 부분 수열의 합
// 하지만 가장 긴 감소하는 부분 수열이어야 하기 때문에 조건식 자체는 동일 - max 구분은 dp[i], dp[j] + board[i]로 변경할 뿐
